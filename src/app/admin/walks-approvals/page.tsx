...
export default function WalksApprovalsPage() {
  ...
  const [pending, setPending] = useState<WalkRow[]>([]);
  const [live, setLive] = useState<WalkRow[]>([]);
    ...
    (async () => {
      ...
      const [p, l] = await Promise.all([
        supabase
          .from("walks")
          .select(
            "id, slug, name, area, distance_km, duration_minutes, difficulty, submitted_at, status"
          )
          .eq("status", "pending")
  ...
  async function postAction(
    path: "/api/admin/walks/approve" | "/api/admin/walks/delete",
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-600">
            Review new walks/routes before they go live.
          </p>
        </div>
            ...
            {pending.map((w) => (
              <li key={w.id} className="flex items-center justify-between py-4">
                <div className="min-w-0">
                  <div className="text-lg font-medium text-slate-900">{w.name}</div>
                  <div className="mt-0.5 text-sm text-slate-600">
                    {w.area ?? "—"} · {w.distance_km ?? "—"}km ·{" "}
                    {w.duration_minutes ?? "—"} mins · {w.difficulty ?? "—"}
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <Link
                    href={`/heritage/walks/${w.slug || w.id}`}
                  ...
                  <button
                    onClick={() =>
                      postAction("/api/admin/walks/approve", { id: w.id })
                    }
                  ...
                  <button
                    onClick={() =>
                      postAction("/api/admin/walks/delete", { id: w.id })
                    }
                    disabled={busyId === w.id}
      ...
      <section className="rounded-2xl border bg-white p-6">
        <h2 className="text-2xl font-bold text-slate-900 mb-3">
          Live walks ({stats.live})
        </h2>

        {live.length === 0 ? (
          <p className="text-slate-500">No live walks yet.</p>
        ) : (
          <ul className="divide-y divide-slate-200">
            {live.map((w) => (
              <li key={w.id} className="flex items-center justify-between py-4">
