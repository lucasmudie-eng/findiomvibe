...
export default function LeaguesApprovalsPage() {
  ...
  const [pending, setPending] = useState<LeagueRow[]>([]);
  const [live, setLive] = useState<LeagueRow[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [p, l] = await Promise.all([
        supabase
          .from("leagues")
          .select("id, slug, name, sport_type, area, submitted_at, status")
          .eq("status", "pending")
  ...
  async function postAction(
    path: "/api/admin/leagues/approve" | "/api/admin/leagues/delete",
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-600">
            Review new leagues / clubs before they go live.
          </p>
        </div>
            ...
            {pending.map((l) => (
              <li key={l.id} className="flex items-center justify-between py-4">
                <div className="min-w-0">
                  <div className="text-lg font-medium text-slate-900">{l.name}</div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() =>
                      postAction("/api/admin/leagues/approve", { id: l.id })
                    }
                  ...
                  <button
                    onClick={() =>
                      postAction("/api/admin/leagues/delete", { id: l.id })
                    }
                    disabled={busyId === l.id}
      ...
      <section className="rounded-2xl border bg-white p-6">
        <h2 className="text-2xl font-bold text-slate-900 mb-3">
          Live leagues ({stats.live})
        </h2>

        {live.length === 0 ? (
          <p className="text-slate-500">No live leagues yet.</p>
        ) : (
          <ul className="divide-y divide-slate-200">
            {live.map((l) => (
              <li key={l.id} className="flex items-center justify-between py-4">
