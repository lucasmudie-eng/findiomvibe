...
const GEOCODE_CACHE_KEY = "manxhive_deal_geocodes_v1";
...
function buildLocationLabel(deal: DealMapItem) {
  const pieces = [deal.businessName, deal.area].filter(Boolean);
...
export default function DealsMap({
  ...
  useEffect(() => {
    ...
    const resolve = async () => {
      ...
      const queued = deals
        .map((deal) => ({
          deal,
          location: buildLocationLabel(deal),
        }))
        .filter((entry) => entry.location.length > 0);

      for (const { deal, location } of queued) {
  ...
  useEffect(() => {
    ...
    markers.forEach((marker) => {
      ...
      if (marker.businessName) detailRows.push(marker.businessName);
      if (marker.locationLabel) detailRows.push(marker.locationLabel);
      if (marker.discountLabel) detailRows.push(marker.discountLabel);
      ...
      const popupHtml = `
        <div style="min-width:180px;font-family:ui-sans-serif,system-ui,-apple-system;line-height:1.35;">
          ...
          ${
            detailRows.length
              ? `<div style="font-size:11px;color:#475569;margin-top:6px;">${detailRows.join(
                  " • "
                )}</div>`
              : ""
          }
    ...
    >
      {!token ? (
        ...
        <div className="flex h-full w-full flex-col items-center justify-center gap-2 bg-slate-50 text-sm text-slate-600">
          ...
          <span className="text-xs text-slate-500">
            Add area or business details to show pins here.
          </span>
        </div>
      ) : (
        <>
